        ...
        <form action="{{ route('websites.new') }}" method="post" enctype="multipart/form-data" id="form-website">
            ...
            <div class="form-group">
                ...
                <div class="input-group">
                    ...
                    <input id="i-stripe-api-key" type="password" class="form-control{{ $errors->has('stripe_api_key') ? ' is-invalid' : '' }}" name="stripe_api_key" value="{{ old('stripe_api_key') }}" placeholder="{{ __('Restricted Stripe API key') }}">
                </div>
                @if ($errors->has('stripe_api_key'))
                    <span class="invalid-feedback d-block" role="alert">
                        <strong>{{ $errors->first('stripe_api_key') }}</strong>
